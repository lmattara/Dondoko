...
│   └── evolutions.json            → { species_name: next_evolution_name (or a name array for branching evolutions) } for the "evolve on Gym win" feature, hand-curated, not regenerated by a script
├── pokedex_data/
...
`data/evolutions.json` isn't in this pipeline, it's hand-curated (branching
evolutions like Eevee keep every option, not just the first) and edited
directly instead.
...
- **Evolution on Gym win** — `data/evolutions.json` (hand-curated, edit
  directly) plus `evolveRandomEligible()` in `game.js`.

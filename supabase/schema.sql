...
-- The friend never needs to be online. Safe to let the client write
-- directly (own row only) — no cooldown/uniqueness/anti-cheat concern, the
-- team is just species names the player already owns in real run history.
...
-- One row per PvP battle fought. `winner_id` comes from the client-side
-- battle outcome (no server-side re-simulation to check against — same
-- trust model as everything else client-computed here, PvP has no
-- rewards/stakes beyond bragging rights and history). 10+ total rows
-- between the same pair (either direction) flips their "Friend" tag to
-- "Rival" (see profile.html/game.js).
